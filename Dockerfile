...
# DB_SSL is intentionally NOT defaulted here. The image now defaults to TLS-on
# (db.ts treats unset as enabled) so a deployment pointed at a managed/remote
# Postgres encrypts in transit by default. Set DB_SSL=false only when the DB is
# a trusted local/sidecar instance with no TLS (the docker-compose path does
# this). For full protection on managed providers also set DB_SSL_STRICT=1 and
# DB_SSL_CA so the server certificate is verified.
