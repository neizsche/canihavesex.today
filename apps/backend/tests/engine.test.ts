...
function log(date: string, spec: LogSpec = {}): Log {
  return {
    ...
    symptoms: [],
    notes: null,
    cycle_start: spec.cycle_start,
    is_uncertain: spec.is_uncertain,
    created_at: date,
  };
}
...
function calendarCycles(
  start: string,
  cycleLen: number,
  n: number
): { logs: Log[]; today: string } {
  const logs: Log[] = [];
  let d = start;
  for (let c = 0; c < n; c++) {
    for (let p = 0; p < 4; p++)
      logs.push(log(addDaysIso(d, p), { bleeding: p === 0 ? 'heavy' : 'medium' }));
...
function engineDaysFromLogs(logs: Log[]) {
  ...
  return logs.map((l) => ({
    ...
    mucusValue: l.mucus
      ? ({ dry: 0, sticky: 1, creamy: 2, watery: 3, eggwhite: 4 } as Record<string, number>)[
          l.mucus
        ]
      : 0,
...
describe('golden: clean symptothermal cycle → confirmed ovulation', () => {
  ...
  const logs: Log[] = [];
  for (let p = 0; p < 4; p++)
    logs.push(log(addDaysIso(start, p), { bleeding: p === 0 ? 'heavy' : 'medium' }));
  ...
  logs.push(
    log(addDaysIso(start, 12), { temperature: 36.4, mucus: 'eggwhite', lh_test: 'positive' })
  );
  logs.push(log(addDaysIso(start, 13), { temperature: 36.45, mucus: 'creamy' }));
  for (let cd = 15; cd <= 22; cd++)
    logs.push(log(addDaysIso(start, cd - 1), { temperature: 36.75, mucus: 'dry' }));
...
describe('golden: light-period segmentation (A1 regression)', () => {
  ...
  test('an uncertain explicit marker is ignored', () => {
    const starts = detectCycleStarts([
      log('2026-02-10', { cycle_start: true, is_uncertain: true }),
    ]);
    assert.equal(starts.length, 0);
  });
});
...
describe('golden: missed-log gap → median robust', () => {
  ...
  const filtered = logs.filter(
    (l) => daysBetweenIso(gapStart, l.date) < 0 || daysBetweenIso(gapStart, l.date) > 4
  );
  ...
  test('a 56-day merged cycle does not poison the median length', () => {
    ...
    assert.ok(
      lengths.some((l) => l >= 50),
      'fixture should contain the merged outlier'
    );
...
describe('detectors', () => {
  test('BBT shift: max(prev6)+0.2 over 3 days, ovulation = day before shift', () => {
    const start = '2026-01-01';
    const logs: Log[] = [];
    for (let cd = 1; cd <= 12; cd++)
      logs.push(log(addDaysIso(start, cd - 1), { temperature: 36.4 }));
    for (let cd = 13; cd <= 16; cd++)
      logs.push(log(addDaysIso(start, cd - 1), { temperature: 36.7 }));
  ...
  test('window geometry holds and status enum is valid for random logs', () => {
    ...
    for (let iter = 0; iter < 200; iter++) {
      ...
      for (let i = 0; i < days; i++) {
        const spec: LogSpec = {};
        if (Math.random() < 0.3)
          spec.bleeding = (['none', 'spotting', 'light', 'medium', 'heavy'] as const)[
            Math.floor(Math.random() * 5)
          ];
        if (Math.random() < 0.3) spec.temperature = 35 + Math.random() * 3;
        if (Math.random() < 0.2)
          spec.mucus = (['dry', 'sticky', 'creamy', 'watery', 'eggwhite'] as const)[
            Math.floor(Math.random() * 5)
          ];
...
describe('lostTrack - overdue cycle handling', () => {
  test('computes lostTrack correctly and drops projected days to unsure/Luteal status', () => {
    ...
    const { statuses: statusesEarly } = runFusionEngine('u', {
      logs,
      meta: META,
      today: todayEarly,
    });
    ...
    const { statuses: statusesOnlyTemp } = runFusionEngine('u', {
      logs: logsWithOnlyTemp,
      meta: META,
      today,
    });
    ...
    const { statuses: statusesOnlyMucus } = runFusionEngine('u', {
      logs: logsWithOnlyMucus,
      meta: META,
      today,
    });
    ...
    const { statuses: statusesTempAndSpotting } = runFusionEngine('u', {
      logs: logsWithTempAndSpotting,
      meta: META,
      today,
    });
    ...
    const { statuses: statusesOnlySpotting } = runFusionEngine('u', {
      logs: logsWithOnlySpotting,
      meta: META,
      today,
    });
