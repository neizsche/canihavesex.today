...
export function logHasMeaningfulData(
  log:
    | Pick<
        Log,
        'bleeding' | 'temperature' | 'mucus' | 'lh_test' | 'disturbances' | 'symptoms' | 'notes'
      >
    | null
    | undefined
): boolean {
...
export class LogRepository {
  ...
  async getLog(userId: string, date: string): Promise<Log | null> {
    const rows = await this.db.query<any>(`SELECT * FROM logs WHERE user_id = $1 AND date = $2`, [
      userId,
      date,
    ]);
