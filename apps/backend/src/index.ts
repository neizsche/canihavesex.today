...
export async function createApp() {
  ...
  if (
    isSelfHost() &&
    (process.env.ENABLE_CLOUD_BILLING === 'true' || createDodoProviderFromEnv())
  ) {
    app.log.warn(
  ...
  // Every other /api/v1 mutation — deleting data/account, changing cycle config
  // or theme, re-anchoring — is blocked so one visitor
