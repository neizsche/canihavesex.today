...
import {
  ...
  generateIsoDateRange as generateDateRange,
  isoToday,
  BACKLOG_WINDOW_DAYS,
} from './utils/dates.js';
...
// =============================================================================
// CONFIG — every tunable constant lives here. Each is covered
...
export const CONFIG = {
  ...
  conf: {
    ...
    LH: 0.8,
    MUCUS: 0.55,
    CALENDAR_REGULAR: 0.4,
    CALENDAR_IRREGULAR: 0.25,
    ALIGN_BONUS: 0.05,
    ALIGN_MAX: 0.15,
...
// =============================================================================
// Guardrails — clamps + soft invariant checks. We *degrade,
...
function isValidTemp(t: number | null | undefined): t is number {
  return (
    typeof t === 'number' &&
    Number.isFinite(t) &&
    t >= CONFIG.bbt.MIN_VALID &&
    t <= CONFIG.bbt.MAX_VALID
  );
}
...
export function runFusionEngine(
  ...
  for (const cycle of relevantCycles) {
    ...
    const activeCycleLength =
      result.ovulationDay > 0 ? result.ovulationDay + personalizedLuteal : predictedLength;
    ...
    const logsAfterThreshold = sortedLogs.filter(
      (l) => l.date > expectedEndDate && l.date >= cycle.start_date
    );
    ...
    const hasFertilitySignal = logsAfterThreshold.some((l) => {
      ...
      const hasLh =
        l.lh_test !== null && l.lh_test !== undefined && String(l.lh_test) !== 'notTaken';
    ...
    const lostTrack =
      !cycle.end_date &&
      cycleDayOfToday > activeCycleLength &&
      !(hasFertilitySignal && hasMenstruationOrCervicalFluid);
    ...
    for (const date of daysToGen) {
      ...
      const { status, phase } = classifyDay({
        ...
        irregular,
        strongHistory:
          cycleStats.completedCount >= CONFIG.cycle.MIN_CYCLES_FOR_STRONG_HISTORY &&
...
export function detectCycleStarts(logs: Log[]): IsoDate[] {
  ...
  for (const log of sorted) {
    ...
    const continuesMenses =
      lastBleedDate !== null &&
      daysBetween(lastBleedDate, log.date) <= CONFIG.cycle.MAX_BLEED_BREAK + 1;
    if (!continuesMenses) {
      accept(log.date);
    }
    lastBleedDate = log.date;
  ...
  for (let i = CONFIG.bbt.BASELINE_DAYS; i < temps.length; i++) {
    ...
    if (next.length >= 2 && next[0].temp > ltl && next[1].temp > ltl && next[1].temp - ltl >= t) {
      return {
        detected: true,
        rule: 0,
        anchorDay: temps[i - 1].cycleDay,
        evalCompleteDay: next[1].cycleDay,
      };
    }
    ...
    if (next.length >= 3 && next.every((d) => d.temp > ltl)) {
      return {
        ...
        rule: 1,
        anchorDay: temps[i - 1].cycleDay,
        evalCompleteDay: next[2].cycleDay,
      };
    }
 ...
 * following days of lower quality (the dry-up).
 */
export function detectMucusShift(days: EngineDay[]): ShiftResult {
  ...
  for (let i = 0; i < mucusDays.length; i++) {
    ...
    return {
      detected: true,
      anchorDay: mucusDays[i].cycleDay,
      evalCompleteDay: following[2].cycleDay,
    };
  }
  return { detected: false };
}

function detectLhSurge(days: EngineDay[]): { anchorDay: number | null; isMultiSurge: boolean } {
  const positives = days.filter((d) => d.isLhPositive);
  if (positives.length === 0) return { anchorDay: null, isMultiSurge: false };
  const last = positives[positives.length - 1];
  const isMultiSurge = positives.length > 2 && last.cycleDay - positives[0].cycleDay > 5;
  return { anchorDay: last.cycleDay + 1, isMultiSurge }; // ovulation ~24-36h after surge
}

// =============================================================================
// Tier B prediction + irregular handling
// =============================================================================

interface IrregularInfo {
  isIrregular: boolean;
  spread: number;
  reason: string | null;
}
  ...
  return cycles
    .filter(
      (c) =>
        c.end_date &&
        c.length &&
        c.length >= CONFIG.cycle.MIN_CYCLE &&
        c.length <= CONFIG.cycle.MAX_CYCLE
    )
    .map((c) => c.length!);
}
...
function detectIrregular(cycles: Cycle[], meta: EngineMeta): IrregularInfo {
  ...
  if (recent.length >= 2) {
    ...
    if (spread > CONFIG.irregular.SPREAD_DAYS)
      return { isIrregular: true, spread, reason: 'spread' };
  }
  ...
  return {
    isIrregular: false,
    spread: recent.length >= 2 ? Math.max(...recent) - Math.min(...recent) : 0,
    reason: null,
  };
}
...
export function calcOvulationDay(
  cycleLength: number,
  luteal: number
): { day: number; isExact: boolean } {
  if (cycleLength < CONFIG.luteal.MIN + CONFIG.ovulation.MIN_DAY) return { day: 0, isExact: false };
  let resolved = resolveLuteal(luteal);
  const maxSupported = cycleLength - CONFIG.ovulation.MIN_DAY;
  if (maxSupported < CONFIG.luteal.MIN) return { day: 0, isExact: false };
  let isExact = true;
  if (resolved > maxSupported) {
    resolved = maxSupported;
    isExact = false;
  }
  const day = cycleLength - resolved;
  if (day < CONFIG.ovulation.MIN_DAY) return { day: 0, isExact: false };
  return { day, isExact };
}

/**
 * Personalized luteal phase from past cycles' confirmed ovulation (BBT shift),
 * averaged, clamped to [10,20], needs ≥2 samples. Falls back to default.
 */
...
function fuse(
  ...
  if (bbt.detected && bbt.anchorDay) {
    signals.push({
      source: 'BBT',
      anchorDay: bbt.anchorDay,
      confidence: CONFIG.conf.BBT,
      explanation: `Temp shift; ovulation CD ${bbt.anchorDay}`,
    });
  }
  ...
  if (lh.anchorDay !== null) {
    signals.push({
      source: 'LH',
      anchorDay: lh.anchorDay,
      confidence: CONFIG.conf.LH,
      explanation: `LH surge → ovulation CD ${lh.anchorDay}`,
    });
    if (lh.isMultiSurge) anomalies.push('Multiple LH Surges (PCOS Risk)');
  }
  ...
  if (mucus.detected && mucus.anchorDay) {
    signals.push({
      source: 'MUCUS',
      anchorDay: mucus.anchorDay,
      confidence: CONFIG.conf.MUCUS,
      explanation: `Peak mucus CD ${mucus.anchorDay}`,
    });
  }
  ...
  const calendarDay =
    calc.day > 0
      ? calc.day
      : Math.max(CONFIG.ovulation.MIN_DAY, Math.round(ctx.predictedLength - CONFIG.luteal.DEFAULT));
  signals.push({
    source: 'CALENDAR',
    anchorDay: calendarDay,
    confidence: ctx.irregular.isIrregular
      ? CONFIG.conf.CALENDAR_IRREGULAR
      : CONFIG.conf.CALENDAR_REGULAR,
  ...
  const ovulationDay = anchorSignal.anchorDay;
  ...
  for (const s of bioSignals) {
    ...
    else if (diff > CONFIG.corroboration.DAYS + 1) {
      conflicts++;
      confidence -= CONFIG.conf.CONFLICT_PENALTY;
    }
  }
  ...
  if (bbt.detected && bbt.anchorDay) {
    const corroborated =
      (mucus.detected &&
        mucus.anchorDay !== undefined &&
        Math.abs(mucus.anchorDay - bbt.anchorDay) <= CONFIG.corroboration.DAYS) ||
      (lh.anchorDay !== null &&
        Math.abs(lh.anchorDay - bbt.anchorDay) <= CONFIG.corroboration.DAYS);
  ...
  if (tier === 'prediction' && ctx.irregular.isIrregular) {
    const span = clamp(
      ctx.irregular.spread || CONFIG.irregular.RANGE_SPAN_MIN,
      CONFIG.irregular.RANGE_SPAN_MIN,
      CONFIG.irregular.RANGE_SPAN_MAX
    );
    winStart = Math.max(1, winStart - span);
    winEnd = winEnd + span;
  }

  // Output invariants — soft-asserted, never thrown in prod.
  ...
  return {
    ...
    confidence,
    signals: bioSignals.length ? bioSignals : signals,
    anomalies,
    isConfirmed,
    tier,
  };
...
function computeCycleStats(cycles: Cycle[]): CycleStats {
  ...
  const avgCycleLength = lengths.length
    ? Math.round(lengths.reduce((a, c) => a + c, 0) / lengths.length)
    : CONFIG.cycle.DEFAULT_LENGTH;

  let variation = 0;
  if (lengths.length > 1) {
    const variance =
      lengths.reduce((a, c) => a + Math.pow(c - avgCycleLength, 2), 0) / lengths.length;
...
function buildDayMeta(
  ...
  const primarySignal: SignalSource =
    result.signals.length > 0 ? result.signals[0].source : 'CALENDAR';
