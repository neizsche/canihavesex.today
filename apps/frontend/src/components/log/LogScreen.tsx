...
import {
  Thermometer, Droplets, Activity, ChevronRight, CheckCircle2, Info, Lock, ArrowRight, TestTube, AlertTriangle, FileText
} from 'lucide-react';
...
export function LogScreen() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      return apiJson<{ found: boolean; payload?: any; }>(`/api/logs/${date}`);
    },
  ...
  // NEW: Log Suggestion Query (Strictly for today)
  const suggestionQuery = useQuery({
    queryKey: ['log-suggestion', date],
    queryFn: async () => {
      return apiJson<{ available: boolean; suggestion?: any; }>(`/api/logs/suggestion?date=${date}`);
    },
    enabled: date === todayIso() && query.isFetched && !query.data?.found,
    retry: false
  });
  ...
  React.useEffect(() => {
    ...
    if (query.data?.found && query.data.payload) {
      ...
      if (suggestionQuery.data?.available && suggestionQuery.data.suggestion) {
        // PREFILL FROM SUGGESTION
        const s = suggestionQuery.data.suggestion;
  ...
  }, [query.data, suggestionQuery.data, date]);
      ...
      <div className="flex-1 w-full min-h-0 overflow-y-auto overflow-x-hidden no-scrollbar pb-20">
        <div className="w-full max-w-md mx-auto min-h-full flex flex-col pt-safe-offset-2 sm:pt-4">
          ...
          {isPrefilled && !isLockedPast && (
            <div className="mx-4 mt-3 mb-1 bg-blue-50/80 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl p-3 flex items-start gap-3 backdrop-blur-sm animate-in fade-in slide-in-from-top-2">
              <Info className="icon-sm text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
              <div className="flex flex-col gap-0.5">
                <span className="text-[14px] font-medium text-blue-700 dark:text-blue-300">Data pre-filled from yesterday</span>
                <span className="text-[13px] text-blue-600/80 dark:text-blue-400/80 leading-snug">
                  Review and tap Save to confirm. Daily analysis starts after valid input.
                </span>
              </div>
            </div>
          )}
