...
export function AnimatedEducationScreen({ title, items, onComplete }: AnimatedEducationScreenProps) {
    ...
    React.useEffect(() => {
        ...
        timers.push(setTimeout(() => setVisibleCount(2), 1500));

        // Show item 3 after delay
        timers.push(setTimeout(() => setVisibleCount(3), 2700));
                            ...
                            {items.map((item, index) => (
                                ...
                                >
                                    <div className="space-y-1">
                                        <h3 className="font-semibold text-[19px] text-zinc-900 dark:text-zinc-100">
                                            {item.title}
                                        </h3>
