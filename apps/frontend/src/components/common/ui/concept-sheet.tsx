...
export function ConceptSheet({
  ...
  return (
    ...
    >
      ...
      {/* Sheet */}
      <div
        className={`w-full max-w-md bg-[#F2F2F7] dark:bg-[#1C1C1E] rounded-t-[20px] sm:rounded-[20px] overflow-hidden shadow-2xl relative transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1) transform ${
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              {Icon && (
                <div
                  className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center shadow-sm backdrop-blur-md ${
                    bgImage
              ...
              <div className="flex flex-col min-w-0">
                {badge && (
                  <span
                    className={`inline-block text-[10px] font-bold uppercase tracking-widest mb-0.5 opacity-80 ${bgImage ? 'text-white' : 'text-zinc-500'}`}
                  >
                    {badge}
                  </span>
                )}
