...
export function ActionModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-xl p-4 animate-in fade-in duration-300">
      <div
        className={`w-full ${widthClass} bg-white p-10 md:p-12 rounded-[2rem] shadow-[0_8px_80px_-12px_rgba(0,0,0,0.25)] border border-slate-200/50 relative overflow-hidden animate-in zoom-in-95 duration-300`}
      >
        <Button
          type="button"
          variant="ghost"
        ...
        <div className="text-center space-y-10">
          ...
          <div className="flex flex-col items-center gap-5">
            <img src="/logo.png" alt="Logo" className="w-20 h-20 mix-blend-multiply" />
            <div>
              <div className="text-2xl font-outfit tracking-tighter text-slate-900 mb-2">
                {BRAND.PREFIX}
                <span className="text-rose-500 font-extrabold italic">{BRAND.HIGHLIGHT}</span>
                {BRAND.SUFFIX}
              </div>
              {title && <div className="mt-3 text-slate-500 font-medium text-lg">{title}</div>}
            </div>
          </div>
