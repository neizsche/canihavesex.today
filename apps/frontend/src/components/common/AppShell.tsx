...
function redirectToAuth(): void {
  // A 401 means the session is gone. Wipe the persisted (session/billing) blob
  // so the next person on this device can't paint a stale logged-in shell.
  clearPersistedCache();
...
function restorePersistedCache(client: QueryClient): void {
  ...
  // Evict any legacy v1 blob on every load — it predates the persistence
  // allowlist and may hold health data we no longer want on disk.
  try {
...
export function AppShell() {
  ...
  React.useEffect(() => {
    ...
    persistQueryClient({
      ...
      // Allowlist: only non-sensitive session/billing queries are written to
      // localStorage. Health queries (logs/insights/calendar/stats) never leave
      // memory. See security-review/17 + privacy-review/13.
      dehydrateOptions: {
