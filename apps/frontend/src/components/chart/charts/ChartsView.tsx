                    ...
                    {trends.length > 0 && (
                        <div className="space-y-3">
                            {trends.map((trend, idx) => (
                                <div key={idx} className="p-4 bg-zinc-50/80 dark:bg-zinc-900/50 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                                    <h3 className="font-semibold text-[15px] text-zinc-900 dark:text-zinc-100">{trend.heading}</h3>
                                    <p className="text-[13px] text-zinc-500 dark:text-zinc-400 mt-1 leading-relaxed">{trend.msg}</p>
                                </div>
                            ))}
                        </div>
                    )}
