...
export function QuickStats({ data }: QuickStatsProps) {
    ...
    // Checkbox state for historical view
                ...
                <div className="bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <div className="text-[17px] font-semibold text-zinc-900 dark:text-white">{data.subText || data.periodStartDate}</div>
                        </div>
                        <div className="text-right">
                            {/* Only show secondary date if relevant */}
                            {data.ovulationDate && (
                                <>
                                    <div className="text-[13px] text-zinc-500 dark:text-zinc-400 font-medium mb-0.5">Probable Ovulation</div>
                                    <div className="text-[17px] font-semibold text-zinc-900 dark:text-white">{data.ovulationDate}</div>
                                </>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between gap-3">
                        ...
                        {data.phase ? (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {isAccurate && <Sparkles className="w-3 h-3 text-white dark:text-zinc-900 fill-current" />}
                                </div>
                                <span className="text-[14px] text-zinc-600 dark:text-zinc-400 font-medium selection:bg-none cursor-pointer" onClick={() => setIsAccurate(!isAccurate)}>
                ...
                <div className="flex items-center">
                    ...
                    <div className="flex-1 pl-6 space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-[15px] text-zinc-500 dark:text-zinc-400 font-medium">{CHARTS_VIEW_LABELS.quickStats.cycleDay}</span>
                            <span className="text-[15px] font-semibold text-zinc-900 dark:text-white font-mono">{data.cycleDay}</span>
                        </div>

                        {/* Fertile Window */}
                        <div className="flex justify-between items-center">
                            <span className="text-[15px] text-zinc-500 dark:text-zinc-400 font-medium">{CHARTS_VIEW_LABELS.quickStats.fertility}</span>
                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full ${data.fertilityStatus === 'High' ? 'bg-rose-100 dark:bg-rose-500/20' : 'bg-zinc-100 dark:bg-zinc-800'}`}>
                                {data.fertilityStatus === 'High' && <Sparkles className="w-3 h-3 text-rose-600 dark:text-rose-400 fill-current" />}
                                <span className={`text-[12px] font-bold ${data.fertilityStatus === 'High' ? 'text-rose-600 dark:text-rose-400' : 'text-zinc-500'}`}>
